  ...
  return (
    <>
      ...
      <button
        onClick={() => setOpen((o) => !o)}
        className={cn(
          "no-print fixed bottom-5 left-5 z-40 flex items-center gap-2 rounded-full bg-primary px-4 py-3 text-primary-foreground shadow-xl transition-all hover:scale-105 hover:shadow-2xl",
      ...
      <div
        className={cn(
          "no-print fixed bottom-5 left-5 z-50 flex w-[min(92vw,400px)] flex-col overflow-hidden rounded-[var(--radius-lg)] border border-border bg-card shadow-2xl transition-all",
          open ? "h-[min(78vh,580px)] opacity-100" : "pointer-events-none h-0 translate-y-4 opacity-0",
        )}
      >
        <div className="flex items-center justify-between border-b border-border bg-gradient-to-l from-primary/10 to-transparent p-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="font-display text-sm font-semibold">העוזר החכם</div>
