...
3. farForModel: מקדם בנייה של **שטח עיקרי בלבד** חלקי שטח המגרש (שטחי שירות אינם חלק מהמקדם — הם נקבעים בנפרד ב-serviceAreaRatio). אם המכרז מציין זכויות עיקריות ושטח מגרש — חשב בדיוק mainRightsSqm/plotAreaSqm. צמוד קרקע 0.35–0.7; בנייה רוויה לפי התב"ע/המכרז, ואם חסר — 2.5–5 לפי אופי האזור.
...
6. כלכלת פריפריה ובנייה נמוכה — כוונן את מבנה העלות, לא רק את המחיר:
   ...
   - serviceAreaRatio: מגדלים ~0.31; בנייה מרקמית נמוכה ~0.22–0.26 (בפריפריה עמוקה בלי מרתפים/לובי — לכיוון 0.20).
   ...
   - **פריפריה עמוקה** (ירוחם, דימונה, מצפה רמון, אופקים, נתיבות, שדרות וכד'): בנייה רוויה מדורגת/מרקמית שם נבנית בפועל ב-cc של 5,600–6,800 ₪/מ"ר (מפרט בסיסי, בלי חניון, קבלנים מקומיים) — אל תשתמש בעלויות גוש-דן. בנוסף, רמ"י משווקת שם עם התחייבות רכישה של המדינה לדירות שלא יימכרו — סיכון המכירות נמוך, קבע requiredMargin 0.10–0.13.
   - בדיקת שפיות פריפריה: מכרזי רוויה בפריפריה העמוקה **נסגרים בפועל** במחירי קרקע חיוביים (נמוכים אך חיוביים). אם ההנחות שלך מייצרות שווי קרקע שלילי עמוק (מיליוני ₪ שליליים) למכרז כזה — מבנה העלות שקבעת כבד מדי למקום; הוזל עלויות ומרווח לפני שאתה מסיק שהפרויקט "לא כלכלי".
...
{"typology":"SINGLE_FAMILY|MULTI_FAMILY","unitsForModel":n,"farForModel":n,"salePricePerSqm":n,"salePriceRationale":"...","constructionCostPerSqm":n?,"parkingCostPerSpace":n?,"bettermentLevyILS":n?,"serviceAreaRatio":n?,"extraObligationsILS":n?,"avgUnitSizeSqm":n?,"requiredMargin":n?,"confidence":"high|medium|low","cautions":["..."]}`,
  ...
  return {
    ...
    constructionCostPerSqm: Number.isFinite(cc) && cc >= 5500 && cc <= 16000 ? Math.round(cc) : undefined,
...
- קצר: headline עד 160 תווים; כל why עד 2 משפטים. תשובה ארוכה נחתכת והופכת ללא-שמישה.
החזר JSON תקין בלבד.`;
...
חשוב: כשהערת ה-FAR אומרת שהמקדם חושב מזכויות הבנייה שבחוברת או נלקח ממנה ישירות — זה הנתון הקובע; הבדל מול הצעת שכבת ההנחות (farForModel) אינו סתירה, אינו באג ואסור שיהיה blocking. המקדם הוא שטח עיקרי בלבד — אל תשווה אותו למקדם הכולל שטחי שירות.
...
export interface CriticContext {
  ...
  farNote?: string;
  /** Where the model's FAR came from — when it's NOT "ai", farForModel was advisory and unused. */
...
הנחות: ${
      ctx.assumptions
        ? JSON.stringify(
            {
              salePricePerSqm: ctx.assumptions.salePricePerSqm,
              // When the engine took its FAR from the booklet (rights/tender/solved),
              // the AI's farForModel was advisory and unused — showing it to the
              // critic produces false "internal contradiction" blocks.
              farForModel: ctx.farSource === "ai" ? ctx.assumptions.farForModel : undefined,
              unitsForModel: ctx.assumptions.unitsForModel,
            },
            null,
            0,
          )
